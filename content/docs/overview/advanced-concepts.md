...
- [**Healthchecks**](/deployments/healthchecks): By default, as soon as your deployment is launched, it's considered "healthy". You can configure an endpoint path under Deploy > Healthcheck Path that Railway will query once it deploys your service. Railway only routes traffic to deployments that pass the health check and keeps the previous deployment active until the new one returns a `200` status code. This enables zero-downtime deployments: your users continue to reach the old version while the new version starts up, and traffic switches over only after Railway confirms the new deployment is healthy. If the health check fails within the configured timeout, the deploy is marked as failed and the previous version continues serving traffic.
