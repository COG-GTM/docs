...
- [**Healthchecks**](/deployments/healthchecks): By default, as soon as your deployment is launched, it's considered "healthy". You can configure an endpoint path under Deploy > Healthcheck Path that Railway queries when it deploys your service. Railway only routes traffic to deployments that pass the health check and keeps the previous deployment active until the new one returns a `200` status code. This enables zero-downtime deployments: your users continue to reach the old version while the new version starts up, and traffic switches over only after Railway confirms the new deployment is healthy. If the health check fails within the configured timeout, Railway marks the deploy as failed and the previous version continues serving traffic.
...
- Deploy <a href="/templates" target="_blank">templates</a>

### Public API

The Railway <a href="/integrations/api" target="_blank">public API</a> is built with GraphQL and is the same API that powers the Railway dashboard. Similar to the CLI, you can interact with your Railway project programmatically by communicating with the API.
